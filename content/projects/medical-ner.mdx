...
- **Problem:** Unstructured biomedical text contains domain-specific entities (diseases, chemicals, genes); manual annotation doesn't scale.
- **Solution:** BioBERT + CRF layer, hyperparameter-tuned with Optuna, versioned in MLflow with full reproducibility.
- **Impact:** 74% F1 on medical entity recognition with 3 percentage points improvement from optimization; production pipeline with monthly retraining.
...
- **Constraint:** Small dataset (2.7K training samples); rare entities had only 50 examples each.
- **Technical Compromise:** Used float32 instead of lower precision to maintain F1; chose quantization over distillation to save development time.
- **Intentionally Not Done:** Didn't implement entity linking or knowledge graph integration; would require external biomedical ontology maintenance.